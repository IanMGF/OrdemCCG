...
import com.ordemrealitas.ccg.cli.CLIPlayer
...
class Match(private val player1: Player, private val player2: Player): EventCause{
    ...
    fun afterCardsArePlayed(){
        ...
        for(cardEntity in hpPlayerCards){
            ...
            eventHandler.callEvent(AddCardToBoardEvent(cardEntity, this, strongestPlayer).apply { cause = strongestPlayer })
        }
...
fun registerCards(match: Match){
    class ImprovisedCard(override val id: String,
    ...
    val kian        = ImprovisedCard("kian", "Kian", 2, 7, action = BiConsumer { event, cardEntity -> run {
        if(event is RoundEndEvent && cardEntity.location == CardEntity.Location.BOARD)
            cardEntity.player.hand.forEach { it.cost-- }
    }})
    val verissimo   = ImprovisedCard("sr-verissimo", "Sr. Veríssimo", 2, 7, action = BiConsumer { event, cardEntity ->
        run {
    ...
    }, description = "Quando sou invocado, dou +2 de força para todos os aliados. Se um aliado fosse ser destruído e eu estiver no deck ou na mão, me invoco.")
    val kaiser      = ImprovisedCard("kaiser", "Kaiser", 1, 1)
    val joui        = ImprovisedCard("joui", "Joui Jouki",3, 5, action = BiConsumer { event, cardEntity ->
        run {
            if(event !is RoundStartEvent)
                return@run
            if(cardEntity.location != CardEntity.Location.BOARD)
                return@run
            val buffCardEvent = ChangeCardEntityEvent(cardEntity, match, (cardEntity.player.board.size - 1).coerceAtMost(3), 0)
    ...
    val arthur      = ImprovisedCard("arthur", "Arthur Cervero", 2, 5, action = BiConsumer { event, cardEntity ->
        run {
    ...
    val dante       = ImprovisedCard("dante", "Dante", 1, 3, action = BiConsumer { event, cardEntity -> run {
    ...
    val elizabeth   = ImprovisedCard("elizabeth", "Elizabeth", 3, 2, action = BiConsumer { event, cardEntity -> run {
        if((event is RoundStartEvent) && cardEntity.hasEffect && cardEntity.location == CardEntity.Location.BOARD)
            cardEntity.run { match.eventHandler.callEvent(PlayerDrawEvent(match, player).apply{ cause = cardEntity }); hasEffect = false; }
    } }, description = "Ao me revelar, compra uma carta no início do próximo turno")
    val gal         = ImprovisedCard("gal", "Gal", 1, 4, action = BiConsumer { event, cardEntity -> run {
        if(event !is RoundStartEvent)
            return@run
        if(cardEntity.location != CardEntity.Location.BOARD)
            return@run
        ...
        if(player.hand.isEmpty())
            return@run
        player.hand.filter { cardInHand -> cardInHand.cost == (player.hand.maxByOrNull { it.cost }!!.cost) }.forEach {
            it.cost--
        }
    }}, description="Ao início de cada turno, reduz o custo da(s) carta(s) mais cara(s) em sua mão em 1")
    val godOfDeath  = ImprovisedCard("deus-da-morte", "Deus da Morte", 2, 7, action = BiConsumer { event, cardEntity ->
        run {
            if(event !is CardRevealEvent)
                return@run
    ...
    val theHost     = ImprovisedCard("anfitriao", "O Anfitrião", 2, 7, action = BiConsumer { event, cardEntity ->
        run {
            if (event !is AddCardToBoardEvent)
                return@run
    ...
    val henri       = ImprovisedCard("henri", "Henri", 7, 2, action = BiConsumer { event, cardEntity -> run {
        if(event !is RoundEndEvent)
            return@run
        if(cardEntity.location != CardEntity.Location.BOARD)
            return@run
    ...
    val cristopher  = ImprovisedCard("cristopher", "Cristopher Cohen", 6, 8, action = BiConsumer { event, cardEntity -> run {
    ...
    val mia         = ImprovisedCard("mia", "Mia", 2, 2, action = BiConsumer { event, cardEntity ->run {
        if(event !is AddCardToBoardEvent)
            return@run
        if(event.cardEntity != cardEntity)
            return@run
    ...
    val enpap       = ImprovisedCard("enpap", "Enpap X", 10, 4, action = BiConsumer { event, cardEntity -> run {
        if(event !is RoundEndEvent)
            return@run
        if(cardEntity.location != CardEntity.Location.BOARD)
            return@run
